...
from datasets.vcoco_text_label import vcoco_obj_text_label_gpt, vcoco_hoi_text_label_gpt
...
class GEN_VLKT(nn.Module):
    ...
    def forward(self, samples: NestedTensor, vlm, is_training=True, clip_input=None):
        ...
        inter_hs, h_hs, o_hs = self.transformer(tgt, mask,
                                                self.query_embed_h.weight,
                                                self.query_embed_o.weight,
                                                self.pos_guided_embedd.weight,
                                                pos[-1], clip_visual)

        # head_feats = h_hs.clone()
        # tail_feats = o_hs.clone()
    ...
    def _set_aux_loss_triplet(self, outputs_hoi_class, outputs_obj_class,
                              outputs_sub_coord, outputs_obj_coord, outputs_inter_hs=None, att_1=None, att_2=None):
...
class SetCriterionHOI(nn.Module):
    ...
    def loss_obj_labels(self, outputs, targets, indices, num_interactions, log=True):
        ...
        loss_obj_ce = F.cross_entropy(src_logits.transpose(1, 2).to('cpu'), target_classes.to('cpu'), self.empty_weight.to('cpu')).to(device)
...
def build(args):
    ...
    if args.with_clip_label:
        ...
        weight_dict['loss_hoi2obj_ce'] = args.obj_loss_coef
        weight_dict['loss_hoi2verb_ce'] = args.hoi_loss_coef
    else:
        weight_dict['loss_hoi_labels'] = args.hoi_loss_coef
        weight_dict['loss_obj_ce'] = args.obj_loss_coef
        weight_dict['loss_hoi2obj_ce'] = args.obj_loss_coef
